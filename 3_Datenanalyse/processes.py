...
def heatfromdoc(doc, city, period, periodunit):
    ...
    #timestamps = list(sorted(doc["starttime"]))
    ...
    timeindex = []
    points = []
    currenttime = doc["starttime"].min()

    while currenttime <= doc["starttime"].max():
        nexttime = currenttime + timedelta64(period, periodunit)

        tripsstartedattime = doc.loc[(doc['starttime'] >= currenttime) &
                                     (doc['starttime'] < nexttime)]

        for trip in tripsstartedattime["route"]:
            for node in trip:
                point = []
        ...
        timeindex.append(str(currenttime))

        points = []

        currenttime = nexttime

    m = Map([48.152764, 11.567963], tiles="cartodbpositron", zoom_start=13)
